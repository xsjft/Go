...
public class GameManger : MonoBehaviour
{
    ...
    private IEnumerator ConnectToServer()
    {
        for (int i = 0; i < serverIPs.Count; i++)
        {
            ...
            if (!connectTask.IsCompleted || !client.Connected)
            {
                ...
                {
                    ShowPopupType2("所有服务器连接失败");
                    yield break;
                }
                else
                {
                    ShowPopupType2("连接超时，尝试下一个服务器");
                }
                continue;
            }

            Debug.Log($"连接成功：{serverIPs[i]}:{serverPorts[i]}");
            break;
        }
    ...
    public IEnumerator LoadServerConfigs()
    {
        ...
        if (req.result == UnityWebRequest.Result.Success)
        {
            ...
            {
                Debug.LogError("解析 StreamingAssets 配置失败: " + e);
            }
        }
        else
        {
            Debug.LogWarning("读取 StreamingAssets 配置失败: " + req.error);
        }

        // ======================
        // 2️⃣ 读取或创建 persistentDataPath 配置
        // ======================
        ...
        else
        {
            ...
            try
            {
                ...
                Debug.Log("本地配置文件不存在，已创建空文件");
            }
            catch (System.Exception e)
            {
                Debug.LogError("创建本地配置文件失败: " + e);
            }
        }

        Debug.Log($"加载完成，共 {serverIPs.Count} 个服务器");
    }
    #endregion
    ...
    private void HandleServerMessage(string msg)
    {
        ...
        {
            OnOpponentExit?.Invoke();
            if (SceneManger.instance.GetScreenName() == "Room") {
                SceneManger.instance.SwitchScene("Room");
            }
            ShowPopupType2("对手退出房间");
            return;
        }

        //请求观战结果
        if (msg.StartsWith("SPECTATE_RESULT:"))
        {
            int result = int.Parse(msg.Substring("SPECTATE_RESULT:".Length)); // 提取idx
        ...
        //接受历史步骤
        if (msg.StartsWith("SPECTATE_OP:"))
        {
            try
            {
                string content = msg.Substring("SPECTATE_OP:".Length); // 去掉前缀
                string[] parts = content.Split(',');                   // 分割成三部分

                if (parts.Length != 3)
                {
                    Debug.LogWarning("SPECTATE_OP 格式错误: " + msg);
                    return;
                }

                int opType = int.Parse(parts[0]);  // 操作类型
                int actorId = int.Parse(parts[1]); // 执行操作的玩家ID
                int pos = int.Parse(parts[2]);     // 落子或悔棋位置

                // 根据 opType 做不同处理
                switch (opType)
                {
                    case 1: // 落子
                        OnLuoziRecived3D?.Invoke(pos);
                        Debug.Log($"观战：玩家 {actorId} 落子位置 {pos}");
                        break;
                    case 2: // 悔棋
                        OnHuiQiSuccess();
                        Debug.Log($"观战：玩家 {actorId} 悔棋");
                        break;
                    case 3: // 过手
                        OnPassTurnRecived();
                        Debug.Log($"观战：玩家 {actorId} 过手");
                        break;
                    case 4: // 认输
                        OnResignRecived();
                        Debug.Log($"观战：玩家 {actorId} 认输");
                        break;
                    default:
                        Debug.LogWarning($"未知操作类型 {opType}");
                        break;
                }
            }
            catch (Exception e)
            {
                Debug.LogWarning($"解析 SPECTATE_OP 失败: {msg}, 错误: {e}");
            }
        }
    }
    #endregion
    ...
    //进入观战
    public void SendSpectate(int targetPlayerId)
    {
        if (!IsOnline) return;

        string msg = $"Spectate:{targetPlayerId}\n";
        byte[] data = Encoding.UTF8.GetBytes(msg);
        stream.Write(data, 0, data.Length);
        Debug.Log(msg);

        Debug.Log($"观战玩家 {targetPlayerId}");
    }
    //退出观战模式
    public void SendExitSpectate()
    {
        if (!IsOnline) return;

        string msg = $"ExitSpectate:{playerId}\n";
        byte[] data = Encoding.UTF8.GetBytes(msg);
        stream.Write(data, 0, data.Length);

        Debug.Log($"玩家 {playerId} 退出观战");
    }
    ...
    public List<GameObject> GetPopups()
    {
        return popups;
    }
