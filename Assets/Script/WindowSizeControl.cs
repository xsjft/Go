...
public class WindowSizeSync : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 获取并缓存主相机（避免Update中重复获取，优化性能）
        mainCamera = Camera.main;
        if (mainCamera == null)
        {
            Debug.LogError("场景中未找到主相机（需勾选Camera的MainCamera标签）");
            return;
        }
        // 初始化相机视野角（确保初始状态适配基准宽高比）
    ...
    void Update()
    {
        ...
        if (Screen.width != lastWindowWidth || Screen.height != lastWindowHeight)
        {
            ...
            if (mainCamera.orthographic == false) // 仅对透视相机生效
            {
                ...
                mainCamera.fieldOfView = fovCorrection;
            }
            ...
            Debug.Log($"窗口已适配：{Screen.width} × {Screen.height}，相机FOV更新为：{mainCamera.fieldOfView:F2}");
        }
    }
}